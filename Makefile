...
build:
	docker build -t ${dockerUser}/${finalImageName}:${finalImageVersion} --build-arg IMAGE_USER=${dockerUser} --build-arg IMAGE_NAME=${buildImageName} --build-arg IMAGE_VERSION=${buildImageVersion} --build-arg HUGO_VERSION=${finalImageVersion} .
...
run:
	...
	@docker run -it --rm --name consul -p 8400:8400 -p 8500:8500 -p 8600:8600/udp ${dockerUser}/${finalImageName}:${finalImageVersion}
